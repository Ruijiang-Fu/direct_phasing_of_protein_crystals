...
program main
  ...
  real,parameter::solvent_content0=0.68 !68.0%
  real,parameter::R_pseudo=0.0
  real,parameter::sigma_weight_initial=8.0
  real,parameter::sigma_weight_final=4.0
  real,parameter::scale_factor=1.0
  integer,parameter::num_of_iterations=20000
  integer,parameter::num_of_iterations_to_limit_den=1500
  ...
  integer,parameter::num_of_origin_choices=4  !P43212 no P41212
  ...
  real::density_on_grid_points_in_ASU_tmp(0:anum/2-1,0:bnum/2-1,0:cnum/2-1)
  real::density_on_grid_points_in_unitcell(0:anum-1,0:bnum-1,0:cnum-1)
  real::weighted_density_on_grid_points_in_ASU(0:anum/2-1,0:bnum/2-1,0:cnum/2-1)
  integer::block_region_in_ASU(0:anum/2-1,0:bnum/2-1,0:cnum/2-1)
  ...
  open(unit=22,file='log_of_R_work_resolution_above_6A.txt',status='replace')

  open(unit=31,file='log_of_R_free_resolution_above_2A.txt',status='replace')
  open(unit=32,file='log_of_R_free_resolution_above_6A.txt',status='replace')

  open(unit=41,file='log_of_CC_and_deltaphi_resolution_above_2A.txt',status='replace')
  open(unit=42,file='log_of_CC_and_deltaphi_resolution_above_6A.txt',status='replace')
  ...
  do
    ...
    if(resolution>=cutoff_resolution)then
      ...
      list_of_uniq_refl(2,h,k,l)=obssf*exp(-(pi*R_pseudo*S)**2)
      if(h>k)then
        list_of_uniq_refl(1,k,h,l)=obsstatus
        list_of_uniq_refl(2,k,h,l)=obssf*exp(-(pi*R_pseudo*S)**2)
      end if
    end if
  end do
  close(10)
  ...
  do iteration=1,num_of_iterations,1
      ...
      do ik=0,knum-1,1
        do ih=0,hnum-1,1
          ...
          * ( cos(2.0*pi*(h*0.5/real(anum)+k*0.5/real(bnum)+l*0.5/real(cnum))) &
          ...
          if(h<k)cycle  !not unique reflections
          ...
          if(abs(calsf)<10.0**(-3))cycle  !not good to find phase
          ...
          if(h>k)then !get all phases for h>=0 k>=0 l>=0, only for convenience
            list_of_uniq_refl(3,k,h,l)=calsf
            phase=mod(l,2)*pi-phase
            phase=mod(phase,2.0*pi)
            if(phase<0)phase=phase+2.0*pi
            list_of_uniq_refl(4,k,h,l)=phase
          end if
        end do
      end do
    end do
   ...
   if(iteration==1 .or. mod(iteration,10)==0)then
    ...
    do while (resolution0>=int(cutoff_resolution))
      if(int(resolution0)/=2 .and. int(resolution0)/=6) then
      ...
      if(int(resolution0)==6)write(22,*)iteration,tab,k_factor,tab,R_value
      resolution0=resolution0-1.0
    end do
   ...
   if(iteration==1 .or. mod(iteration,10)==0)then
    ...
    do while (resolution0>=int(cutoff_resolution))
      ...
      if(int(resolution0)==6)write(32,*)iteration,tab,k_factor,tab,R_value
      resolution0=resolution0-1.0
    end do
    ...
    !monitor mean phase error deltaphi
   if(iteration==1 .or. mod(iteration,10)==0)then
    ...
    do while (resolution0>=int(cutoff_resolution))
      ...
      if(int(resolution0)==2)write(41,*)iteration,tab,delta_phi(1),tab,delta_phi(2),tab,delta_phi(3),tab,delta_phi(4)
      if(int(resolution0)==6)write(42,*)iteration,tab,delta_phi(1),tab,delta_phi(2),tab,delta_phi(3),tab,delta_phi(4)
      resolution0=resolution0-1.0
    end do
...
!          if(h==0 .and. k==0 .and. l==0)cycle  !special reflection F000
          ...
          if(obsstatus/=1)then
            list_of_uniq_refl(3,h,k,l)=sum_of_obssf/sum_of_calsf*calsf
                                      !=k_factor*calsf
            !get all magnitudes for h>=0 k>=0 l>=0, only for convenience
            if(h>k)list_of_uniq_refl(3,k,h,l)=sum_of_obssf/sum_of_calsf*calsf
                                            !=k_factor*calsf
          end if
        end do
      end do
    ...
    =int(anum/2*bnum/2*cnum/2*(1.0-solvent_content))
    ...
    =nint( real(anum/2*bnum/2*cnum/2*(1.0-solvent_content))/real(num_of_units) )
    ...
    if( iteration==num_of_iterations )then
      ...
      do l=0,lnum/2,1
        do k=0,knum/2,1
          do h=0,hnum/2,1
            if(h<k) cycle  !not unique reflections for P43212 space group
            resolution=list_of_uniq_refl(5,h,k,l)
            if(resolution>=cutoff_resolution)then
              obsstatus=nint(list_of_uniq_refl(1,h,k,l))
              obssf=list_of_uniq_refl(2,h,k,l)
    ...
    if( iteration==num_of_iterations .or. iteration==num_of_iterations-1000 )then
        ...
        do ib=0,bnum/2-1,1
          do ia=0,anum/2-1,1
            if(weighted_density_on_grid_points_in_ASU(ia,ib,ic)  &
              ...
              y=cache(2)
              z=cache(3)
              call convert_fractional_coordinates_to_orthogonal_coordinates  &
    ...
    if( iteration==num_of_iterations .or. iteration==num_of_iterations-1000 )then
      ...
      num_of_grid_points_shown_in_ASU=int(anum/2*bnum/2*cnum/2*0.05)  !!!
      ...
      do ic=0,cnum/2-1,1
        do ib=0,bnum/2-1,1
          do ia=0,anum/2-1,1
            if(density_on_grid_points_in_ASU(ia,ib,ic)<cutoff_density)cycle
            cache(1)=real(ia+0.5)/real(anum)
            cache(2)=real(ib+0.5)/real(bnum)
            cache(3)=real(ic+0.5)/real(cnum)
            x=cache(1)
